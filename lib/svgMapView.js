...
function handleMapCountryClick(state, geoName) {
...
export function createSvgMapState(container, viewBox) {
    ...
    const markerPositionByGeoName = new Map();
    ...
    setupPanAndZoom(svg, parseViewBox(viewBox), handleMapZoomChange.bind(null, state), handleMapBackgroundClick.bind(null, state), handleMapCountryClick.bind(null, state));
    return state;
}
function handleCountryPointerEnter(state, geoName) {
    setHoveredCountry(state, geoName);
}
function handleCountryPointerLeave(state) {
...
function renderMarker(state, marker, onMarkerClick) {
    ...
    const context = { state, group, marker, onMarkerClick };
...
export function setMarkersVisible(state, visible) {
...
export function registerCountryClickHandler(state, callback) {
...
export function setSelectedCountry(state, geoName) {
    for (const path of state.selectedCountryPaths)
...
export function setHoveredCountry(state, geoName) {
    ...
    if (position)
        showTooltip(state.tooltip, position.x, position.y, position.label);
    else
...
function handleBackgroundClickOnSvg(state, event) {
...
function createPanZoomState(svg, initialViewBox, onZoomChange, onBackgroundClick, onCountryClick) {
    const state = {
        svg,
...
function setupPanAndZoom(svg, initialViewBox, onZoomChange, onBackgroundClick, onCountryClick) {
