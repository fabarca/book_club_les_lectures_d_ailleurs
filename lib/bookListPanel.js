...
function findContinentGroup(filterTree, continent) {
    return filterTree.find((group) => group.continent === continent);
}
function getSelectedFilterDisplay(filterTree, selectedFilter, totalBookCount) {
    if (selectedFilter.kind === "all")
        return { label: ALL_COUNTRIES_LABEL, count: totalBookCount };
...
function buildFilterTreeHtml(filterTree, selectedFilter, totalBookCount) {
    const items = [
