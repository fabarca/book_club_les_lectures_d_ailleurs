...
export function renderBookListPanel(books, filterOptions, selectedGeoName, onFilterChange, onSelectBook) {
    const panel = document.getElementById("book-panel");
    if (!panel)
        return;
    const selectedLabel = (selectedGeoName && filterOptions.find((option) => option.geoName === selectedGeoName)?.label) ||
        ALL_COUNTRIES_LABEL;
    const optionsHtml = [`<li data-geo="">${ALL_COUNTRIES_LABEL}</li>`]
        .concat(filterOptions.map((option) => `<li data-geo="${escapeHtml(option.geoName)}">${escapeHtml(option.label)}</li>`))
        .join("");
  ...
  `;
    ...
    filterOptionsList?.querySelectorAll("li").forEach((item) => {
        item.addEventListener("click", () => {
            onFilterChange(item.dataset.geo || null);
        });
    });
    panel.querySelectorAll("#book-list li").forEach((item) => {
        item.addEventListener("click", () => {
