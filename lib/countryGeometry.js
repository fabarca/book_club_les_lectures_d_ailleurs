...
function widestSpanAtY(ring, targetY, preferredX) {
    const xs = [];
    for (let i = 0; i < ring.length; i++) {
        const [x1, y1] = ring[i];
        const [x2, y2] = ring[(i + 1) % ring.length];
    ...
    for (let i = 0; i + 1 < xs.length; i += 2) {
        ...
        if (preferredX >= span[0] && preferredX <= span[1])
            return span;
...
export function computeCentroid(geometry) {
    ...
    const centroid = { lat: centroidLat, lng: rewrapLng(finalLng) };
    return centroid;
}
